...
#include<GL/gl.h>
#include<unistd.h> 
int i,n,size,j,t,s[2000][2],l1,t2,flag,f1,k,pos;  // 2000x2 size array, yes I am insane!
float x[5],y[5],a[3];
struct data
{
int x;
...
void search(int p,int q)  // Compare slopes of lines p & q
{
pos=-1;
for(k=0;k<n;k++)
{
...
f1=0;
    for(i=0;i<=size;i++)
        ...
        printf("%d %d\t:Color : %f %f %f \n",j,i,a[0],a[1],a[2]);            
			...
			if(((a[0]==1)&&(a[1]==1)&&(a[2]==1))||f1==0)  // Skipping Horizontal Lines	
			{    
				f1=1;        
				s[t][0]=j;
            	s[t++][1]=i;       
...
float edgedetect(int o,int t)  // calculates slope
{
float temp;
	if(d[o].y==d[t].y)
	return 0;
...
void paint()  // fills pixel by pixel
{
glColor3f(1,0,0);
for(i=0;i<t;)
{	
...
void slope()
{
// calling edgedetect to compute slope
...
// Won't work when (0,0) or (size,size) as input...too lazy to fix it...
if(d[i].x==0)
d[i].x++;
if(d[i].y==0)
d[i].y++;
